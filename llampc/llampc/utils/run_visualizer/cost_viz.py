...
class LLAMPCVisualizer:
    ...
    def __init__(self, filepath, show_all_models=False):
        ...
        Args:
            filepath: Path to .npz file containing MPC log data
            show_all_models: If True, show predictions from all models (can be slow)
        """
    ...
    def load_data(self, filepath):
        """Load MPC data from npz file."""
        data = np.load(filepath, allow_pickle=True)
        
        # Time series data (convert from nanoseconds to seconds)
        time_ns = data["time"]
        self.time = (time_ns - time_ns[0]) * 1e-9  # Shape: (n_timesteps,) in seconds
        
        # Actual executed trajectory
        self.actual_state = data["state"]  # Shape: (n_timesteps, state_dim)
        
        # Predicted states from dynamics bank (ONE-STEP predictions)
        # Shape: (n_timesteps,) - each element is array of shape (num_models, state_dim)
        predicted_states_raw = data["states"]
        
        # Convert to proper 3D array: (n_timesteps, num_models, state_dim)
        ...
        # One-step costs for each model
        # Shape: (n_timesteps, num_models)
        self.one_step_cost = data["one_step_cost"]
        
        # Running costs for each model  
        # Shape: (n_timesteps, num_models)
        self.running_cost = data["running_cost"]
        
        # Control inputs
        self.ctrl = data["ctrl"]  # Shape: (n_timesteps, 2) - [accel, steer]
        
        # Model parameters
        self.params = data["params"]  # Shape: (n_timesteps, num_params)
        ...
        # Get best model index at each timestep (lowest running cost)
    ...
    def setup_figure(self):
        ...
        # Main trajectory plot (left side, top 2 rows)
        ...
        # Model distribution plot (top right)
        ...
        # Running cost over time (middle right)
        ...
        # Prediction error over time (bottom right)
    ...
    def setup_artists(self):
        ...
        # Best model one-step prediction
        self.best_pred_point, = self.ax_traj.plot(
            [], [], 'rs', markersize=10, label='Best Model Prediction', zorder=5
        )
        
        # Arrow showing prediction
        self.prediction_arrow = None
        
        # All model predictions (if enabled)
        self.all_pred_points = []
        if self.show_all_models:
            for _ in range(min(10, self.num_models)):  # Limit to 10 for visibility
                point, = self.ax_traj.plot([], [], 'o', alpha=0.3, markersize=6, color='orange')
                self.all_pred_points.append(point)
        
        # Direction arrow for actual position
        ...
        init_costs = self.running_cost[0]

        self.model_bars = self.ax_models.bar(
            model_indices, init_costs, color='blue', alpha=0.6
        )
        ...
        # Prediction error over time (compute it)
        self.pred_errors = np.zeros(self.n_frames - 1)
        for i in range(self.n_frames - 1):
            best_idx = self.best_model_idx[i]
            pred = self.predicted_states[i, best_idx, :2]  # predicted [x, y]
            actual_next = self.actual_state[i + 1, :2]  # actual next [x, y]
    ...
    def update_frame(self, frame_idx):
        ...
        # Actual trajectory trail
        self.actual_trail.set_data(
            self.actual_x[:frame_idx + 1],
        ...
        # === Direction arrow (STILL FancyArrow — replace with quiver later) ===
        ...
        if pred_dx * pred_dx + pred_dy * pred_dy > 1e-4:
        ...
        # Model distribution bars (cached costs)
    ...
    def build_frame_cache(self):
        ...
        for i in range(self.n_frames):
            ...
            if i < self.n_frames - 1:
                err = np.linalg.norm(pred[:2] - self.actual_state[i + 1, :2])
            else:
                err = 0.0
            ...
            self.frame_cache.append({
                ...
                "pred_y": pred[1],
                "best": best,
                "costs": self.running_cost[i],          # reference, not copy
...
def main():
    ...
    filepath = os.path.join(dir_path, 'stuff2.npz')
    
    # Create visualizer
    visualizer = LLAMPCVisualizer(
        filepath,
        show_all_models=False  # Set to True to see multiple model predictions
    )
    visualizer.show()
