...
class LBHistory:
    ...
    def __init__(self, num_models, history_length, dt, cost_weights, state_size, integrator_factory, dynamics_bank, diffeq,
        ...
        self.last_predicted_states = jax.device_put(jnp.zeros((self.num_models, state_size), dtype='float16'), device = gpu)

        self.running_cost = jax.device_put(jnp.zeros(self.num_models, dtype='float16'), device = gpu)
        self.cost_history = jax.device_put(jnp.zeros((self.num_models, self.history_length), dtype='float16'), device = gpu)
        self.queue_index = 0
        self.dt = dt
        self.cost_weights = jax.device_put(jnp.array(cost_weights, dtype='float16'), device = gpu)
    ...
    def update_lookback_error(self, x_t):
        t0 = time.perf_counter_ns()
        gpu_x = jax.device_put(jnp.array(x_t, dtype = 'float16'), device = gpu)
        t1 = time.perf_counter_ns()
    ...
    def reset(self):
        ...
        self.last_predicted_states = jax.device_put(
            jnp.zeros((self.num_models, self.state_size), dtype='float16'), 
            device=gpu
        )
        self.current_best_model = 0
